...
class PtolShell(QWidget):
    ...
    def _ptolemy_command(self, command: str):
        ...
        if cmd == 'face':
            ...
            _write('[DataInput] starting…', '#9988ff')
            try:
                from Philadelphos.data_input import parse_and_run
                ainur = getattr(self, '_ainur', None)
                parse_and_run(command, ainur_instance=ainur)
                _write('[DataInput] complete.', '#00ff88')
            except Exception as ex:
