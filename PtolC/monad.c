...
char *monad_speak(Monad *m, const char *query, int max_tokens, int verbose)
{
    ...
    /* ── Affect-modulated J weighting ───────────────────────────────────────
     * When irritated (affect > 0): amplify Phase 1 (RevEmrg) zeros — the
     * reactive, confrontational phase enters the field.
     * When passive (affect < 0): suppress all but the deepest Phase 3.    */
    if (m->affect != 0.0f) {
        for (int i = 0; i < m->N; i++) {
            if (J[i] <= 0.0) continue;
            double gam = m->zeros[i];
            int ph = dirac_phase(gam);
            if (m->affect > 0.0f && ph == 1) {
                /* irritated: lift Phase 1 (RevEmrg) proportional to affect */
                J[i] *= 1.0 + (double)m->affect * 3.0;
            } else if (m->affect < 0.0f && ph == 3) {
                /* passive: only the sharpest Emerger peaks survive */
                double sharpness = fabs(sin(gam * 0.5) * cos(gam * 0.5));
                J[i] *= sharpness * (1.0 + (double)(-m->affect));
            }
        }
    }
     ...
     * J magnitude.
     * Phase gate is affect-aware:
     *   affect ≤ 0.5: Phase 3 only (neutral/calm — Emerger, positive)
     *   affect > 0.5: Phase 1 OR Phase 3 (irritated — reactive or positive) */
    double J_max = 0.0;
    for (int i = 0; i < m->N; i++)
    ...
    for (int step = 0; step < m->N && written < max_tokens; step++) {
        ...
        int in_gate;
        if (m->affect > 0.5f) {
            /* irritated: accept Phase 1 (RevEmrg) or Phase 3 (Emrg) */
            in_gate = (s < 0.0 && c < 0.0) || (s > 0.0 && c > 0.0);
        } else {
            /* neutral/passive: Phase 3 only */
            in_gate = (s > 0.0 && c > 0.0);
        }
        if (!in_gate)                                continue;
